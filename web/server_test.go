...
// Test_TzRequestTestData is a test that uses the testdata.json file
// generated by the build/update command
func Test_TzRequestTestData(t *testing.T) {
	var tests []map[string]any
	err := helpers.LoadJSON("../db/testdata/coordinates.json", &tests)
	assert.NoError(t, err)
	...
	for _, tt := range tests {
		...
		tz := tt["tz"].(string)
		lat := fmt.Sprint(tt["lat"].(float64))
		lon := fmt.Sprint(tt["lon"].(float64))

		name := fmt.Sprintf("tz/%s/%s -> %s", lat, lon, tz)

		wantReply := fmt.Sprintf(`{"coords":{"lat":%s,"lon":%s},"tz":"%s"}`, lat, lon, tz)
		...
		t.Run(name, func(t *testing.T) {
			...
			if assert.NoError(t, server.handleTzRequest(c)) {
				assert.Equal(t, http.StatusOK, rec.Code)
				assert.Equal(t, wantReply, strings.TrimSpace(rec.Body.String()))
			}
		})
	}
}
