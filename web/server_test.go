...
func TestNewServer(t *testing.T) {
	settings := ConfigSchema{
		Tz: TzSchema{
			...
			DatabaseName: "../tzdata/timezone.db",
		},
	}
...
func Test_TzVersion(t *testing.T) {
	...
	versionJSON := `{"version":"2023d","url":"https://github.com/evansiroky/timezone-boundary-builder/releases/tag/2023d","geo_data_url":"https://github.com/evansiroky/timezone-boundary-builder/releases/download/2023d/timezones-with-oceans.geojson.zip"}`

	// Assertions
	if assert.NoError(t, server.handleTzVersion(c)) {
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.Equal(t, versionJSON, strings.TrimSpace(rec.Body.String()))
	}
}
